...
class BaselineDataset():
   ...
   def __init__(self, opt, isTrain):
      ...
      # temp_data = torch.load('example.pth')
      temp_data = torch.load('data/UBFCDataset.pth')
      # temp_data = torch.load('UBFCDataset.pth')
      if self.isTrain:
         self.maskset = temp_data['mask'][:17]
         self.dataset = temp_data['image'][:17]
         self.ppg_dataset = temp_data['ppg'][:17]
...
if __name__ == "__main__":
   ...
   b = BaselineDataset(opt, isTrain=False)
